...
- **Конфликты при вставке группы спутников считаются пофайлово**
  (найдено 2026-08-25). `MainViewModel.WithCompanions()` расширяет список
  путей перед batch-операцией, а `BatchExecutor` про группы не знает: если в
  приёмнике лежат и `Sprite.png`, и `Sprite.png.meta`, диалог замены
  покажется дважды на одну логическую операцию. Нужен признак группы в
  `BatchExecutor` (пары «основной + спутники» вместо плоского списка), чтобы
  ответ на конфликт распространялся на всю группу. См. PLAN.md, I2.
