...
| `Wander.Core.Tests` | `net10.0` | xUnit. Тестирует **только** Core через фейки. 552 теста. |
...
стоит троттл на `DispatcherTimer`: флаг «папка изменилась» ставится на
UI-потоке, повторяющийся таймер раз в 500 мс сбрасывает его перечитыванием
и останавливает себя, когда сбрасывать нечего. Повторяющийся, а не
...
или пока идёт своя файловая операция. Ошибка вотчера (переполнение его
буфера) считается изменением — состояние папки после неё неизвестно, и
перечитать её тем более нужно — и вотчер переподнимается на той же папке.

---

## Поиск: два разных механизма под одним полем

Поле над списком делает две несовместимые вещи, и они разведены по разным
классам, а не по флагу внутри одного.

```
             ┌── shallow ──▶ SearchController (Core)      ── на каждую букву
Поле ввода ──┤                └─ проекция снапшота папки, отмена по keystroke
             └── deep ─────▶ ContentSearchController (App) ── по Enter
                              └─ ContentSearchService (Core)
                                   ├─ обход дерева + IContentExtractor
                                   └─ IIndexedSearch (область «весь компьютер»)
```

Граница проходит по `ContentSearchController.IsDeep`: включено содержимое
или область не «эта папка». Пока false — набор идёт в `SearchController`,
как и раньше. Как только true, `SearchController.Query` очищается и поле
...
`TextProbe` уже по байтам, потому что список расширений здесь неизбежно врёт
в обе стороны (`.asset` бывает и YAML, и бинарём).
...
непрочитанным. Дорогие — те же, что специфичные; поэтому «формат, который мы
брались открыть, и не смогли» отличается от «`.dll` по дороге», и счётчик в
строке состояния не тонет в шуме.
...
`APPLY_INDEX_ATTRIBUTES` возвращает **пустой документ**, если задан хоть один
флаг канонизации. Замерено на этой Windows: `1|2` и `1|2|8` дают ноль
...
### `IIndexedSearch` — чужой индекс вместо своего

Единственный случай, где скан не годится, — «где-то на этой машине». Там
отвечает Windows Search: `Search.CollatorDSO` через ADODB с поздним
связыванием (`WindowsSearchIndex`), 33 мс на запрос. ADODB, а не
`System.Data.OleDb`, — чтобы не тянуть пакет ради «открыть, выполнить,
пройти по одной колонке».

Две ловушки, обе в коде с комментариями: берётся `System.ItemUrl`, а не
`System.ItemPathDisplay` (второй локализован — на русской Windows вернёт
`C:\Пользователи\…`, путь, который читается и не открывается), и
подстановочные знаки `LIKE` экранируются (иначе запрос «50%» находит на
машине всё).

Доступность индекса спрашивается **в фоне**: первый вопрос к каталогу стоит
71 мс, а читают это свойство и восстановление сессии, и биндинги панели —
оба на UI-потоке.
...
строку на экране читает ровно одно место, и параллельная таблица по путям
...
- в помеченной папке автоматика молчит. Пометки живут сеанс: переживающая
  перезапуск настройка вида на папку — это `desktop.ini` (PLAN H1), другая
  задача с форматом файла внутри.

### Фон галереи — палитра, а не цвет

`GalleryBackground` (Light / Grey / Dark) — значение в Core;
`GalleryPaletteConverter` в App превращает его в кисти для трёх ролей:
фон, подпись, приглушённый текст. Один конвертер, потому что роли обязаны
двигаться вместе: тёмный фон со светлой темой подписи — это не тёмная тема,
а нечитаемая. Отсюда же прицел: это первый кусок тёмной темы из Roadmap, и
переключение палитры области расширяется, а один захардкоженный цвет
пришлось бы выдирать.

Серый по умолчанию — не вкус: на белом фотография читается темнее и
контрастнее, чем она есть, на чёрном светлее, а нейтральный средний серый
сбивает глаз меньше всего.
...
| Кто | За что отвечает |
|---|---|
| `MainWindow` | Тулбар, адресная строка, статус-бар, деревья, плашка перетаскивания, глобальные хоткеи, сборка контекстного меню, геометрия окна |
...
view-model, чтобы файловая операция шла тем же единственным путём, который
её логирует, проверяет `SystemPathGuard` и делает откатываемой. Один
...
**`Ctrl` + `2`** пользуется тем, что `NavigationSource` уже лежит в истории:
...
двойной клик, восстановление сессии) — берётся панель, в которой клавиатура
была последней (`_lastFolderPane`, обновляется тем же `OnZoneFocusChanged`).
`Ctrl` + `Shift` + `E` — то же самое без переключения.
