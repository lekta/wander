...
`docs/screenshots/*.webp`; шаблон страницы и CSS — `tools/site/page.html`,
`site.css`. Выход (`--out`, по умолчанию `artifacts\site`): `index.html`,
`guide/<слаг>/index.html`, `versions/index.html`, `img/`; перед записью
чистятся только эти папки.

**Структура GUIDE = структура сайта.** `#` — заголовок руководства, текст
под ним — только для GitHub. `##` без `###` — самостоятельная страница;
`##` с `###` — группа: заголовок в навигации, своей страницы нет, текст под
ним запрещён. `###` — страница, `####` — её разделы: оглавление сверху, на
странице они `h2` с линией над каждым. Над заголовком страницы — группа и
«назад / дальше». Абзац, который открывается жирной фразой, — начало темы:
класс `topic`, отбивка сверху. В боковом содержании группы сворачиваются
(`<details>`, без скрипта): раскрыта группа открытой страницы, остальные
свёрнуты — без скрипта свёрнутое читателем на следующую страницу не
переносится. Названия групп и страниц — в одну строку содержания, около
...
клавиш»). Списки расширений — только на странице «Форматы».

**Ссылки.** В GUIDE — как для GitHub: `#якорь` (github-slugger: строчные,
пунктуация выброшена, пробел — дефис, повтор — `-1`, `-2`) генератор
переводит в страницу и раздел сайта; якорь группы ведёт на её первую
...
подпапке Pages, в корне и из файла. Широкие таблицы прокручиваются внутри
колонки (`display: block; width: max-content`).

**Проверки** — ненулевой выход, ничего не пишется: нарушение структуры,
дубль слага или якоря, ссылка в никуда (якорь, файл, картинка не из
`docs/screenshots/*.webp`), незнакомый плейсхолдер; когда источники чисты
— проход по готовым страницам: каждый относительный `href` / `src` и `#id`
...
`docs/app_ico.png`, 64×64 без потерь: значок вкладки у всех страниц, а на
лендинге — у заголовка, вдвое крупнее, с `image-rendering: pixelated`.
Генератор картинки не трогает: у сборки сайта нет зависимостей, кроме
Markdig.
