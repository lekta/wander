...
Контейнер строки (`ListBoxItem`) в плиточных видах не рисует ничего —
шаблон заменён на голый `ContentPresenter`. Контейнер это **вся ячейка**, и
пока подсветку рисовал он, ряд выделенных файлов выглядел одним сплошным
полотном: границ между соседями не видно, посчитать выделенное нельзя.

Подсветка переехала внутрь плитки — `TileHighlight`, `Border` без
содержимого, лежащий в том же `Grid` **под** содержимым. Именно соседом, а
не рамкой вокруг: рамка участвует в раскладке и забрала бы по пикселю с
каждой стороны у ячейки, размер которой `TileMetrics` считает точно, а
подпись в неё влезает впритык.
...
- **Результат фоновой работы приземляется ниже ввода.** Листинг папки
  (`RefreshFolderAsync` делает `Dispatcher.Yield(Background)` перед
  `PublishRows`) и миниатюры (`AsyncIcon`, размеры Medium/Large) ставятся
  на `Background`: что бы ни делал пользователь — оно раньше. Устаревшее
  приземление отбрасывают токен и epoch-проверка.
...
- **Список не чистится на входе в папку.** Старые строки стоят до прихода
  нового листинга (обычно < 150 мс, один своп вместо двух Reset); если
  листинг медленный — чистка происходит вместе со спиннером, чтобы
  устаревшие строки не оставались кликабельными.
