...
странице они `h2`. Порядок — порядок в файле. Слаг страницы — транслит
заголовка, уникален среди страниц; якорь `####` — транслит, уникален на
странице («Полный экран» — и страница, и раздел «Горячих клавиш»).
Списки расширений — только на странице «Форматы».
...
лендинга, шаблона и руководства ведёт в сайт. Шаг «site» в `check.bat`; в конце —
размеры против бюджета: страница руководства 10–30 КБ, лендинг с
...
`docs/app_ico.png`, 64×64 без потерь; на лендинге он вдвое крупнее, с
`image-rendering: pixelated`. Генератор картинки не трогает: у сборки
