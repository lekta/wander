...
Зарегистрированы пока только правила «дописывается» (`.meta`, `.pp3`);
ветка «заменяет» реализована и покрыта тестами, ждёт первого формата (I5).
...
**Осталось:** конфликты по-прежнему считаются по каждому файлу, а не по
группе. Если в приёмнике лежат и `Sprite.png`, и `Sprite.png.meta`, диалог
конфликта покажется дважды. Нужен явный признак группы в `BatchExecutor`.
...
#### I4. Редактирование оценок в `.pp3` — ~~[P2, требует особой аккуратности]~~ — **сделано для `Rank`**
...
**Осталось:** `ColorLabel` только показывается, не редактируется. Создание
`.pp3` по явному согласию пользователя — отдельная задача.
...
- **XMP (`.xmp`)** — самый стандартный сайдкар вообще: Adobe Bridge/Lightroom,
  ...
  делать «оценки фото» всерьёз, XMP важнее pp3 по охвату. Технически это
  первый потребитель ветки «заменяет расширение» — она уже готова.
