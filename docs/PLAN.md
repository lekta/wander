...
### ~~Спринт 6 — ворох визуальных проблем~~ — **закрыт** (2026-08-26)

Взят вне очереди: накопился список того, что видно глазом каждый день.
Подробности того, как это работает теперь, — в [DONE.md](DONE.md).

- **Левая панель.** Перемычка между закладками и деревом (запоминается),
  зона «+» вместо текстовой полосы, подсказка при перетаскивании в
  закладки, подсветка и клик по всей строке дерева, drag папки из дерева,
  правый клик по узлу как цель операций, `Shift` + колесо.
- **Список файлов.** Быстрое перечисление, виртуализация плиточных
  режимов (`Wander.Core/Layout/` + `Controls/VirtualizingWrapPanel`,
  46 тестов на арифметику раскладки и на размеры ячейки), скроллбар больше
  не запускает рамку выделения. Размер ячейки считается из настроек
  (`TileMetrics`), а не подсматривается у случайной плитки, поэтому
  перестал скакать; контейнеры при прокрутке переиспользуются, миниатюры
  кэшируются уже декодированными — прокрутка папки с RAW перестала
  подвисать. Заодно появились замеры (`PerfLog` + `UiStallWatch`), по
  которым это и чинилось.
- **Чистка #3 (частично).** `MainWindow.xaml.cs` 2129 → 1203 строк:
  список файлов и панель просмотра стали отдельными контролами
  (`Views/FileListView`, `Views/PreviewPane`), жесты — отдельными
  классами (`Util/RubberBandController`, `Util/ListVisuals`). Новый режим
  отображения теперь добавляется внутри `FileListView`, а не правкой окна.
- **Мелочи.** Клик по пустому месту адресной строки, `Ctrl` + `Shift` + `C`
  пишет путь, ориентация превью `.CR3`, настройка «не показывать системные
  папки в корне дисков».

Что осталось открытым — в «Технических чистках» ниже и в
...
Интерфейс русский целиком, все 255 строк — в
...
- **Чистка #3 — после спринта 4** (приятное P2): когда A3 (Inline rename),
  ...
  - ~~Разнести `MainWindow.xaml.cs`~~ — **сделано в спринте 6**, 2129 → 1203
    строк. Список файлов уехал в `Views/FileListView`, панель просмотра
    (вместе с image-zoom и video) — в `Views/PreviewPane`, рамка выделения —
    в `Util/RubberBandController`, вопросы к визуальному дереву — в
    `Util/ListVisuals`. **Осталось**: drop-target (~200 строк
    `OnDragOver` / `OnDrop` / `ResolveDropTarget` / подсветка) и
    tree-gestures всё ещё в окне — их стоит вынести, когда деревья
    станут отдельным контролом.
...
- **[P2] A1. Spacing и размеры иконок.** В Tiles 220×40 — иконка 32 px, в LargeIcons —
  ...
  **Частично сделано** для LargeIcons: значения вынесены в `AppSettings`
  ...
  и настраиваются вживую. С 2026-08-26 из них считается `TileMetrics` —
  одно значение и для панели, и для шаблона; размеры Tiles там же, но
  пока константами, так что «настраиваемость у каждого вида» — это
  дописать поля в настройки и прокинуть их в `TileMetrics.ForTiles`.
