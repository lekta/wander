...
1. **Баги и идеи** — [Issues](https://github.com/lekta/wander/issues)
   (шаблоны для bug report и feature request).
2. **Код** — форк → ветка → Pull Request в `master`.
3. Перед PR — `tools\check.bat` (сборка, `dotnet format`, тесты); кодстиль —
...
Смысл: **после принятия PR его код становится частью проекта, которым Lekta
распоряжается так же свободно, как собственным**, включая коммерческие
версии, — чтобы приложение оставалось под единым контролем правообладателя.
