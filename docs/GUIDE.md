...
`Ctrl` + `F` после щелчка по тексту, коду или RTF — поле поиска в правом
верхнем углу: «3 из 17», `Enter` / `Shift` + `Enter` по совпадениям, `Esc`
закрывает. Файл из поиска по содержимому открывается с этим запросом на
первом совпадении, клавиатура остаётся в списке.
...
резкость и гистограмма — про свой снимок; видны на 30 %, пока мышь не
подойдёт к низу или не сменится оценка. Футер внизу один на двоих:
...
| Программы и библиотеки (`.exe`, `.dll`, `.sys`, `.msi`, `.ocx`, `.scr`, `.cpl`, `.drv`) | Карточка: версия, продукт, компания, платформа (x64, консольная, .NET…), подпись — чья и действительна ли, авторские права |
...
`Shift` оставляют на экране левый или правый. В левом нижнем углу снимка —
звёзды, метки, резкость, хелперы и **RAW**; пока мышь не внизу, они видны
на 30 %. Цифры ставят оценку снимку (из двух — тому, что под мышью).
`Esc` или `Enter` — назад; после одного снимка список стоит на последнем
показанном.
...
| Микро-гистограмма | **Гистограмма** по каналам под кнопками (в сплите и на полном экране — у каждого снимка своя); яркая полоса у края — сколько кадра ушло в недосвет (слева) и пересвет (справа), проценты в подсказке |
