...
прав), — плашкой у нижнего края. `Z` — лупа 1:1, которая держится без
кнопки: мышь водит по кадру, у пары вторая половина следует, ПКМ двигает
один снимок; `Z` снова — снять. `Esc` или `Enter` — назад; после одного
снимка список стоит на последнем показанном.
...
«Память под картинки (МБ)» — общий потолок для миниатюр и кадров панели
просмотра, 0 — шестнадцатая часть памяти компьютера (16 ГБ → 1 ГБ,
подсказка называет число), выключатель дискового кэша, «Очистить кэш» с
текущим размером. Пределы действуют сразу, без перезапуска.
...
| **Миниатюры** | Кэш на диске и его предел, «Память под картинки (МБ)» — миниатюры и кадры панели просмотра вместе, «Очистить кэш» |
