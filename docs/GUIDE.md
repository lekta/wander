...
(первый уровень, до 200 строк); выделили файл **внутри** архива — показывает
его содержимое, распаковав временную копию. Файлы больше 32 МБ так не
...
| Обе | «оба»: копия получит имя вида «имя (1).txt»; для папок — слияние |
...
| Раздел | Что там |
|---|---|
| **Основное** | Последняя папка при запуске, автообновление списка, быстрое чтение первых файлов при открытии папки (значки того, что на экране, раньше остального), стрелки в дереве, подтверждение при удалении в корзину и при перемещении, «не спрашивать про одинаковые файлы» |
