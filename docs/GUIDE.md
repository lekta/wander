...
| `Delete` | Удалить **в корзину**; на своей закладке спросит, убрать закладку или папку |
...
| Протяжка по пустому месту | Рамка выделения; появляется, только когда курсор поехал, — клик в щель между плитками остаётся кликом по фону |
...
и `Esc` — отмена. Удалённая из дерева папка исчезает из ветки сразу; если
...
порядок приезжает со следующим обновлением.
...
«Извлечь…», «Создать ярлык», «Копировать путь», «Открыть в терминале».
...
| RAW | Превью из RAW (JPEG) — снимок, который камера положила внутрь файла | ничего — сам Wander |
...
**координаты и высота**, рейтинг, копирайт, название и ключевые слова
переходят в JPEG и TIFF — и из RAW тоже, они читаются из самого
контейнера. В PNG, BMP и GIF метаданные не пишутся: их кодерам некуда.
...
JPEG как есть, без пережатия и за доли секунды: размер — какой положила
камера (у CR3 — 1620×1080, у CR2 и ARW — полный); картинка лежит так, как
сняла камера, а в файле записано, как её повернуть, плюс камера, дата
съёмки и координаты.
...
показываются, — картинка делится пополам: верхний по списку сверху (или
слева, если панель шире, чем выше), футер внизу один на двоих, кнопка
...
вход. Перетащить что-то *на* архив в дереве нельзя.
...
его отменяет; «Извлечь…» спрашивает папку; «Открыть» распаковывает файл во
...
`Ctrl` + `V` в любой программе, перетаскивание из списка в чужое окно.
...
занятым файлом внутри — так же). Замена при
...
нельзя было потерять из виду. Крестик, `Alt` + `F4` и `Esc` сворачивают,
а по завершении окно закрывается само.
...
журнал не пишется: это не событие, а описание списка.
