...
Открыли папку из закладок — «Компьютер» помнит, где вы были: строка там
остаётся подсвеченной бледным цветом, и `Ctrl` + `1` из закладок
возвращает на неё, а не раскрывает дерево до папки из закладок (со
«стрелками, открывающими папку» — сразу в неё). Если в «Компьютере»
ничего не подсвечено, дерево раскрывается до текущей папки, как раньше.
...
относятся к папке под курсором, а список выделение отпускает —
подсвеченный набор на экране всегда один.
...
помечены часами на значке, а панель просмотра пишет «Идёт: …» или «В
очереди: …».
