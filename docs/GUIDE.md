...
прописывается. Настройки, журнал и кэш миниатюр живут в
...
Подробно — «Клавиатура и области окна».

![Главное окно: слева закладки и дерево дисков, в середине папка крупными значками, справа панель просмотра с видео](screenshots/main.webp)
...
просмотра, поиск по содержимому с масками, спутники Unity `.meta` одной
строкой с файлом, свои команды в меню «Действия», «Открыть в терминале».

**Основные сценарии.**

- По папкам ходят деревом и закладками; дерево не сворачивается само,
  раскрытые ветки переживают перезапуск — «Папки: компьютер и закладки».
- Папку смотрят в подходящем виде: таблица, плитки, крупные значки,
  галерея; выбор закрепляется за папкой — «Область файлов».
...
кликом по пустому месту панели папок. Это руководство открывает пункт
«Помощь» в том же меню. Если что-то не так — раздел «Если что-то пошло не
так»: журнал действий и журнал запуска.
...
списком показывает то же, что и окно; окно растягивается, держится поверх
...
| **Спутники** `.xmp` `.pp3` `.meta` | одной строкой с хозяином | оценка и метка; GUID | | | |
...
![Галерея снимков с оценками на тёмном фоне и панель просмотра: RAW под лупой, данные съёмки, звёзды и цветные метки](screenshots/preview.webp)
