...
- P-12. `Relocated(A\B → C\B)`, открыта `A\B` → папка `C\B`, место `C\B` в
  ...
  Строка уходит из уровня `A`, курсор и раскрытое — за ней, и когда
  перечитывание `A` приходит после перехода (баг, найден и закрыт
  2026-09-23).
...
- P-21. Подсветка — своя у каждой панели, не больше строки на панель;
  переход из одной панели строку другой не трогает — в обе стороны
  (решение человека 2026-09-23; до того переход не из закладок гасил их
  строку); активная подсветка на окне одна — инварианты, проверяются после
  каждого события таблицы [В2]. (Было до 2026-09-22: «одна подсветка на
  обе панели».)
- P-22. Открыта папка из закладок, `Ctrl+1` из закладок в «Диски» →
  ...
  папке на диске»). То же в обратную сторону, из «Дисков» в закладки
  (решение человека 2026-09-23): `Ctrl+1` ходит туда-обратно между двумя
  папками; из списка — только раскрытие открытой папки, без перехода.
  `Ctrl+Shift+E` по-прежнему раскрывает открытую папку в её панели.
