...
- Иконки и превью файлов: картинки и RAW, GIF, видео, музыка с обложкой и проигрывателем, 3D-модели (STL, OBJ, glTF), документы (PDF, HTML, MHT, RTF, FB2), markdown, текст и код с подсветкой. На плитках — обложка альбома и обложка книги, а не общий значок. В планах — превью всего, что может читать система
...
- Хоткеи: `Alt+←/→/↑`, `F5`, `Del`, `Ctrl+C/X/V`, `F2` — переименование прямо в строке, `Ctrl+F` — поиск, `Ctrl`+колесо — размер значков, буквы — переход к файлу по имени
- **Поиск**: быстрый фильтр по имени на каждую букву (с масками `*.cs;*.xaml`, а через двоеточие — и по тексту: `*.cs:бюджет`) и отдельное окно поиска — маска имени **и** текст внутри файлов, с подпапками или без, запускается само. Читает текст в любой кодировке (кириллица в Windows-1251 и DOS-866 тоже), `.docx` / `.xlsx` / `.pptx` / `.epub` / OpenDocument, а `.doc`, `.rtf` и `.pdf` — системными обработчиками Windows. Своего индекса Wander не строит и на диске ничего не оставляет
...
- **Режим галереи** для папок со снимками: крупные картинки, настраиваемый фон (светлый / серый / тёмный), звёзды и цветные метки прямо на миниатюре. Включается сам в папке, где больше половины файлов — изображения, и не перебивает вид, выбранный руками
- Фильтр по звёздам и цветным меткам — полосой над списком, появляется только там, где в папке есть оценённые файлы. Столбец «Оценка» в таблице сортируется кликом по заголовку
- Оценка снимка, у которого не было файла-спутника: он создаётся одним щелчком, с подтверждением и с отменой по `Ctrl+Z`. По умолчанию `.xmp`, потому что появившийся `.pp3` меняет то, как снимок открывается в RawTherapee
