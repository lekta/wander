...
- **Поиск**: по имени — фильтром на каждую букву, по содержимому файлов и по подпапкам — по `Enter`. Читает текст в любой кодировке (кириллица в Windows-1251 и DOS-866 тоже), `.docx` / `.xlsx` / `.pptx` / `.epub` / OpenDocument, а `.doc`, `.rtf` и `.pdf` — системными обработчиками Windows. Область «весь компьютер» отвечает индекс Windows; своего индекса Wander не строит и на диске ничего не оставляет
