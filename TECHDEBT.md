...
- **«Этот компьютер» без иконки** — синтетический bookmark-узел имеет
  `FullPath=""`, и `IconConverter` для пустого пути возвращает null.
  Текст «Этот компьютер» отображается без значка слева. Когда появится
  `IShellNamespace` (D5), у узла будет parsing name CLSID-а My Computer,
  и `IIconProvider` сможет вернуть системную иконку. Пока — голый текст.
